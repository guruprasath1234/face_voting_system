...
scale_percent = 75  # Resize to 75% of original size
width = int(imgBackground.shape[1] * scale_percent / 100)
height = int(imgBackground.shape[0] * scale_percent / 100)
imgBackground = cv2.resize(imgBackground, (width, height))
...
while True:
    ...
    # Resize the video frame to fit the smaller background
    frame_resized = cv2.resize(frame, (400, 300))

    # Shift camera feed 20% from the top of the background
    offset_y = int(imgBackground.shape[0] * 0.2)
    offset_x = 100  # Keep x same as before
    imgBackground[offset_y:offset_y + 300, offset_x:offset_x + 400] = frame_resized
    ...
    if output is not None:
        ...
        if k == ord('3'):
            ...
            if exist:
                with open("Votes.csv", "a") as csvfile:
                    writer = csv.writer(csvfile)
                    attendance = [output[0], "TVK", date, timestamp]
                    writer.writerow(attendance)
            else:
                with open("Votes.csv", "a") as csvfile:
                    writer = csv.writer(csvfile)
